...
use Portable\FilaCms\Models\Navigation;
use Portable\FilaCms\Filament\Resources\NavigationResource\Pages;
        ...
        return $form
            ->schema([
                ...
                Forms\Components\Group::make()
                    ->schema([
                        ...
                        Forms\Components\Select::make('reference_content')
                            ...
                            ->getSearchResultsUsing(fn (string $search, Get $get, NavigationResource $resource): array => $resource->getContents($search, $get))
                            ->getOptionLabelUsing(fn (string $value, NavigationResource $resource, Get $get): ?string => ($resource->getSourceModel($get('reference_page')))->select('id', 'title')->where('id', $value)->first()?->title)
                            ->required()
                            ->searchable()
                            ->columnSpan(2),
                ...
                Forms\Components\Select::make('parent_id')
                    ...
                    ->options(function (NavigationResource $resource) use ($model) {
                        // do not show self, if edit
                        return Navigation::when(gettype($model) === 'object', function ($query) use ($model) {
